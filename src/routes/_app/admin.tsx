...
function AdminDashboard() {
  ...
  const enrollStudent = async () => {
    ...
    const { error } = await supabase
      .from("enrollments")
      .insert({ user_id: enrollUser, course_id: enrollCourse });
    if (error) return toast.error(error.message);
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="content">
        ...
        <TabsContent value="certificates" className="mt-4">
          <CertificatesPanel users={users ?? []} courses={courses ?? []} />
        </TabsContent>
                ...
                <tr key={u.id} className="border-b last:border-0">
                  ...
                  <td className="p-3">
                    <div className="flex flex-wrap gap-1">
                      {(u.roles.length ? u.roles : ["student"]).map((r: string) => (
                        <Badge
                          key={r}
                  ...
                  <td className="p-3 text-right">
                    {!u.roles.includes("instructor") ? (
                      <Button size="sm" variant="ghost" onClick={() => promote(u.id, "instructor")}>
                        + Instr
                      </Button>
                    ) : (
                      <Button size="sm" variant="ghost" onClick={() => demote(u.id, "instructor")}>
                        − Instr
                      </Button>
                    )}
                    {!u.roles.includes("admin") ? (
                      ...
                      <Button size="sm" variant="ghost" onClick={() => demote(u.id, "admin")}>
                        − Admin
                      </Button>
                    )}
                  </td>
...
function AssessmentsPanel() {
  const { data, isLoading } = useQuery<AssessmentAttempt[]>({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .select(
          "id, score, total, passed, created_at, user_id, lesson_id, profiles:user_id(display_name, avatar_url), lessons(title, courses(title))",
        )
        .order("created_at", { ascending: false })
        .limit(200);
  ...
  const avg = Math.round(
    (data.reduce((a, d) => a + (d.total ? d.score / d.total : 0), 0) / data.length) * 100,
  );
    ...
    <div className="space-y-3">
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <div className="glass rounded-2xl p-4">
          ...
          <p className="font-display text-2xl font-semibold">{passed}</p>
        </div>
        <div className="glass rounded-2xl p-4">
          <p className="text-xs text-muted-foreground">Average score</p>
          <p className="font-display text-2xl font-semibold">{avg}%</p>
        </div>
      </div>
      <div className="glass overflow-x-auto rounded-2xl">
        <table className="w-full text-sm">
...
function RequestsPanel() {
  ...
  const approve = async (r: EnrollmentRequest) => {
    ...
    const { error: e1 } = await supabase
      .from("enrollments")
      .insert({ user_id: r.user_id, course_id: r.course_id });
    ...
    <div className="space-y-4">
      <div className="glass space-y-3 rounded-2xl p-5">
        <h3 className="flex items-center gap-2 font-semibold">
          <Award className="h-4 w-4 text-primary" /> Issue certificate manually
        </h3>
        ...
        <Button onClick={issue} className="bg-gradient-brand text-primary-foreground border-0">
                ...
                <tr key={c.id} className="border-b last:border-0">
                  ...
                  <td className="p-3 text-right">
                    <Button
                      ...
                      onClick={() =>
                        downloadCertificate({
                          name: c.profiles?.display_name ?? "Student",
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        const subject = encodeURIComponent(
                          `Your certificate — ${c.courses?.title ?? "JewelIQ Academy"}`,
                        );
                        const body = encodeURIComponent(
                          `Congratulations ${c.profiles?.display_name ?? ""}!\n\nYour certificate for "${c.courses?.title}" is ready.\n\nVerification: ${verifyUrl}\nCertificate ID: ${c.cert_code}\n\n— JewelIQ Academy`,
                        );
    ...
    <div className="grid gap-4 lg:grid-cols-[280px_1fr]">
      <aside className="rounded-2xl border bg-card p-3">
        ...
        <ul className="space-y-1">
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block text-[10px] uppercase tracking-wide text-muted-foreground">
                    {c.category} · {c.is_published ? "published" : "draft"}
                  </span>
                </span>
              </button>
            </li>
          ...
          {courses.length === 0 && (
            <p className="px-2 py-4 text-center text-xs text-muted-foreground">No courses yet.</p>
          )}
        </ul>
      </aside>
        ...
        {selected ? (
          <div className="rounded-2xl border bg-card p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="min-w-0">
                <Badge variant={selected.is_published ? "default" : "outline"}>
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-lg">
        ...
        <div className="space-y-3">
          ...
          <div>
            <label className="text-xs font-medium">Thumbnail hue (0–360)</label>
...
function ModulesPanel({ courseId }: { courseId: string }) {
  ...
  const handleDragEnd = async (e: DragEndEvent) => {
    ...
    qc.setQueryData(
      ...
      reordered.map((m, i) => ({ ...m, order_index: i })),
    );
    await Promise.all(
      reordered.map((m, i) =>
        supabase
              ...
              {modules?.map((m) => {
                ...
                return (
                  <SortableModuleRow
                    ...
                    module={m}
                    open={open}
                    onToggle={() => setOpenIds({ ...openIds, [m.id]: !open })}
                    onEdit={() => {
                      setEditing(m);
                      setShowDialog(true);
                    }}
...
function LessonsPanel({ courseId, moduleId }: { courseId: string; moduleId: string | null }) {
  ...
  const handleDragEnd = async (e: DragEndEvent) => {
    ...
    qc.setQueryData(
      ["admin-lessons", courseId, moduleId],
      reordered.map((l, i) => ({ ...l, order_index: i })),
    );
    await Promise.all(
    ...
    <div>
      <div className="mb-2 flex items-center justify-between">
        <p className="text-xs font-medium text-muted-foreground">
          Lessons {lessons?.length ? `(${lessons.length}) — drag to reorder` : ""}
        </p>
        <Button
          size="sm"
          variant="outline"
...
function SortableLessonRow({
  ...
  return (
    ...
    >
      ...
      <Button size="sm" variant="ghost" title="Assessment" onClick={onQuiz}>
        ...
        <div className="space-y-3">
          ...
          <div>
            <label className="text-xs font-medium">
              Content (text or markdown — split into pages with blank lines)
            </label>
            <Textarea
              value={content}
              onChange={(e) => setContent(e.target.value)}
              rows={12}
              placeholder="Write the lesson content here. Paragraphs are paginated automatically for the next-button reading flow."
            />
          </div>
        </div>
        <DialogFooter>
...
function QuizDialog({ lesson, onClose }: { lesson: Lesson; onClose: () => void }) {
  ...
  const handleDragEnd = async (e: DragEndEvent) => {
    ...
    qc.setQueryData(
      ["admin-quizzes", lesson.id],
      reordered.map((q, i) => ({ ...q, order_index: i })),
    );
    await Promise.all(
      reordered.map((q, i) => supabase.from("quizzes").update({ order_index: i }).eq("id", q.id)),
    );
      ...
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <ClipboardCheck className="h-4 w-4 text-primary" /> Assessment · {lesson.title}
          </DialogTitle>
        </DialogHeader>

        <div className="space-y-3">
          <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
            Existing questions (drag to reorder)
          </p>
          {isLoading ? (
                ...
                <ul className="space-y-2">
                  {quizzes.map((q, i) => (
                    <SortableQuizRow
                      ...
                      quiz={q}
                      index={i}
                      onTogglePublished={() => togglePublished(q)}
